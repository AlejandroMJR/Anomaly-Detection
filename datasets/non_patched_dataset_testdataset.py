...
class NoiseprintDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.is_train is True and self.model != "efficient":
            #x = x.crop((8, 8, 8 + self.min_w, 8 + self.min_h))
            x = x.crop((0, 0, 0 + self.min_w, 0 + self.min_h))

            x = self.transform_x(x)
            #x = torch.zeros_like(x)
        elif self.is_train is False and self.model != "efficient":
            # w, h = x.size
        ...
        if y == 0:
            ...
            if mask.mode =="RGBA":
                mask = mask.convert("L")
           # mask = mask.crop((left, top, w - (7 - left), h - (7 - top)))
    ...
    def load_dataset_folder(self):
        ...
        if not self.is_train:
            ...
            gt_fpath_list = [os.path.join(gt_dir, img_fname + '.png')
                             for img_fname in img_fname_list]
            #gt_fpath_list = ["/nas/home/ajaramillo/projects/datasets/dso-dsi/DSO-1-Fake-Images-Masks/splicing-12.png"]
